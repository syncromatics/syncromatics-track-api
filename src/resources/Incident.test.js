...
describe('When dispose an incident', () => {
  const client = new Client();
  beforeEach(() => mockIncidents.setUpSuccessfulMock(client));
  beforeEach(() => fetchMock.catch(503));
  afterEach(fetchMock.restore);
  ...
  it('should resolve the promise with false alarm', () => {
    const promise = new Incident(client, Incident.makeHref('SYNC', 1)).disposeIncident(1);
    return promise.should.be.fulfilled;
  });
  it('should resolve the promise with Cleared', () => {
    const promise = new Incident(client, Incident.makeHref('SYNC', 1)).disposeIncident(2);
    return promise.should.be.fulfilled;
  });
});
